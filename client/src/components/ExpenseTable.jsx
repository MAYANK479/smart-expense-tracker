...
import { 
  Search, ArrowUpDown, Edit3, Trash2, Calendar, CreditCard, Download, ArrowUpRight, TrendingDown
} from 'lucide-react';
...
export default function ExpenseTable({
  ...
  const exportToCSV = () => {
    ...
    const headers = ['Type', 'Date', 'Title / Merchant', 'Category', 'Amount', 'Payment Method', 'Notes', 'Tags'];
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '14px', marginBottom: '20px' }}>
        <div>
          ...
          <p style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}>
            Showing {sortedExpenses.length} income & expense entries
          </p>
        </div>
                ...
                return (
                  ...
                  >
                    ...
                    <td style={{ padding: '14px' }}>
                      <span className={`badge ${isIncome ? 'badge-emerald' : 'badge-indigo'}`}>
                        {isIncome ? <ArrowUpRight size={12} /> : <TrendingDown size={12} />}
                      ...
                      {exp.tags && (
                        <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap', marginTop: '4px' }}>
                          {exp.tags.split(',').map((t, i) => (
                            <span key={i} style={{ fontSize: '0.68rem', color: isIncome ? '#6ee7b7' : '#a5b4fc', background: isIncome ? 'rgba(16,185,129,0.12)' : 'rgba(99,102,241,0.12)', padding: '2px 6px', borderRadius: '4px' }}>
                              #{t.trim()}
                            </span>
                          ))}
                        </div>
                    ...
                    {/* Amount */}
                    <td style={{ padding: '14px', textAlign: 'right', fontWeight: 700, color: isIncome ? '#34d399' : '#ffffff', fontSize: '0.95rem' }}>
                      {isIncome ? '+' : '-'}{formatCurrency(exp.amount, currencySymbol)}
                    </td>
