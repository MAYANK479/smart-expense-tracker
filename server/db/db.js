...
let memoryExpenses = [
  { id: 1, user_id: 1, type: 'income', title: 'Software Dev Monthly Salary', amount: 4500.00, category: 'Salary & Earnings', date: '2026-08-01', payment_method: 'Bank Transfer', notes: 'Primary payroll direct deposit', tags: 'income,salary' },
  { id: 2, user_id: 1, type: 'income', title: 'Freelance UI/UX Consulting', amount: 850.00, category: 'Freelance & Business', date: '2026-08-03', payment_method: 'Bank Transfer', notes: 'Client web project milestone', tags: 'income,freelance' },
  { id: 3, user_id: 1, type: 'expense', title: 'Supermarket Groceries', amount: 142.50, category: 'Food & Dining', date: '2026-08-01', payment_method: 'Credit Card', notes: 'Weekly household restocking', tags: 'groceries,essential' },
  { id: 4, user_id: 1, type: 'expense', title: 'Electric Bill', amount: 88.20, category: 'Utilities', date: '2026-08-02', payment_method: 'Bank Transfer', notes: 'Monthly utility bill', tags: 'bills,home' },
  { id: 5, user_id: 1, type: 'expense', title: 'Uber Rides', amount: 34.00, category: 'Transportation', date: '2026-08-03', payment_method: 'Credit Card', notes: 'Commute to office', tags: 'commute' },
  { id: 6, user_id: 1, type: 'expense', title: 'Coffee & Bakery', amount: 18.50, category: 'Food & Dining', date: '2026-08-04', payment_method: 'Apple Pay', notes: 'Morning team coffee', tags: 'snacks' },
  { id: 7, user_id: 1, type: 'expense', title: 'Gym Membership', amount: 65.00, category: 'Health & Fitness', date: '2026-08-05', payment_method: 'Credit Card', notes: 'Monthly subscription', tags: 'fitness' },
  { id: 8, user_id: 1, type: 'expense', title: 'Cloud Hosting Subscription', amount: 49.00, category: 'Services & Tech', date: '2026-08-06', payment_method: 'Credit Card', notes: 'AWS / Vercel servers', tags: 'work,recurring' },
  { id: 9, user_id: 1, type: 'expense', title: 'Dinner with Friends', amount: 115.00, category: 'Food & Dining', date: '2026-08-06', payment_method: 'Cash', notes: 'Italian restaurant', tags: 'leisure' },
  { id: 10, user_id: 1, type: 'expense', title: 'Amazon Electronics purchase', amount: 199.99, category: 'Shopping', date: '2026-08-06', payment_method: 'Credit Card', notes: 'Noise cancelling headphones', tags: 'gadgets' }
];
...
export const db = {
  ...
  // EXPENSE & INCOME QUERIES
  async getAllExpenses({ userId, type, category, search, startDate, endDate, sortBy = 'date', sortOrder = 'DESC' } = {}) {
    if (isPostgresConnected && pool) {
      try {
        let sql = "SELECT id, user_id, COALESCE(type, 'expense') as type, title, amount::float, category, to_char(date, 'YYYY-MM-DD') as date, payment_method, notes, tags FROM expenses WHERE 1=1";
        const params = [];
        let pIndex = 1;
    ...
    if (type) {
      result = result.filter(e => (e.type || 'expense') === type);
    }
    if (category && category !== 'All') {
  ...
  async addExpense({ userId = null, type = 'expense', title, amount, category, date, payment_method, notes, tags }) {
    ...
    if (isPostgresConnected && pool) {
      try {
        const sql = `
          INSERT INTO expenses (user_id, type, title, amount, category, date, payment_method, notes, tags)
          VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
          RETURNING id, user_id, type, title, amount::float, category, to_char(date, 'YYYY-MM-DD') as date, payment_method, notes, tags;
        `;
        const res = await pool.query(sql, [userId, txnType, title, formattedAmount, category, formattedDate, payment_method || 'Card', notes || '', tags || '']);
        return res.rows[0];
      } catch (err) {
